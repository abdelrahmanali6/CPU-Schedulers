...
public class preemptiveShortestJobFirst {
    ...
    public preemptiveShortestJobFirst(ArrayList<Process> pr, Double cs) 
    {
    	processes = pr;
    	contextSwitching = cs;
	...
	public void scheduling( ) 
	{
		...
		while (true) 
        {
            ...
            {
				System.out.println("Context Switching: " + (int)currentTime + " -> " + (int)(currentTime+contextSwitching));
	...
	private Process optimalProcess(int CurrntTime)//1
    {
		...
		for (int i=0 ; i<tempArrivedProcesses.size()-1 ; i++) 
		{
			...
			{
				if (tempArrivedProcesses.get(j).ProcessBurstTime < tempArrivedProcesses.get(least).ProcessBurstTime)
				{
					least = j;
				}
			}
